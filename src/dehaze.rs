...
/// 影片模組的「亮芯羽化」起始值。比照片模組寬：
/// 影片整支只調一組設定，沒辦法一格一格看著核心調，寬一點站在「煙火簇周圍的
/// 光暈留完整」那一側——光暈被切掉在動起來時特別顯眼，而多留的那一圈煙是
/// 靜態的、比較不容易察覺。實拍 4K 煙火影片比下來 60 最穩（使用者 2026-09-16 裁定）
pub const MOVIE_CORE_FEATHER: i32 = 60;
